...
function comNombre1(&$errores)
{
    if (textoVacio('usuario')) {
        $errores['usuario'] = "este campo esta vacio o es erroneo";
    }
}
function comContra1(&$errores)
{
    if (textoVacio('contraseña')) {
        $errores['contraseña'] = "este campo esta vacio o es erroneo";
    }
...
function registrarUsuario()
{
    try {
        ...
        $sql = 'INSERT INTO Usuarios (usuario, clave, nombre, correo, perfil, fecha_nacimiento) 
                VALUES (?, ?, ?, ?, ?, ?)';
        $stmt = $con->prepare($sql);

        // Ejecutar la consulta con los valores proporcionados
        $stmt->execute([$_REQUEST['usuario'], $_REQUEST['contra'], $_REQUEST['nombre'], $_REQUEST['email'], 'USR', $_REQUEST['fecha']]);

        // Cerrar la conexión y retornar true si la inserción fue exitosa
        unset($con);
        return true;
    } catch (PDOException $e) {
        // Manejo de errores: Imprimir mensaje de error y retornar false
